...
local sea_level = df_caverns.config.sunless_sea_level
...
minetest.register_on_generated(function(minp, maxp, seed)
	...
	if minp.y > df_caverns.config.sunless_sea_level or maxp.y < df_caverns.config.sunless_sea_min then
		return
	end
	...
	for z = z_min, z_max do -- for each xy plane progressing northwards
		for y = y_min, y_max do -- for each x row progressing upwards
			if y <= df_caverns.config.sunless_sea_level then
